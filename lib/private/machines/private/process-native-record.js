...
module.exports = function processNativeRecord(nativeRecord, WLModel, meta) {
  if(!(_.isObject(nativeRecord) && !_.isArray(nativeRecord) && !_.isFunction(nativeRecord))) {
    console.log('1st argument must be a dictionary', nativeRecord);
  }

  if(!(!_.isUndefined(WLModel))) {
    console.log('2nd argument is required', WLModel);
  }
